            ...
            <div className="mb-6">
              ...
              <ul className="divide-y divide-rose-100 border border-rose-100 rounded-xl">
                {selected.items.map((it) => {
                  const variantLabel = [it.size, it.color_name].filter(Boolean).join(' · ');
                  return (
                    <li key={it.id} className="p-3 flex justify-between text-sm gap-3">
                      <div className="min-w-0 flex-1">
                        <div>{it.product_name} <span className="text-ink/50">× {it.quantity}</span></div>
                        {variantLabel && (
                          <div className="text-xs text-ink/60 mt-0.5 flex items-center gap-1.5">
                            {it.color_hex && (
                              <span
                                className="inline-block w-2.5 h-2.5 rounded-full border border-rose-200"
