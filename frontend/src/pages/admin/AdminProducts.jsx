                ...
                <tr key={p.id} className="hover:bg-cream/50">
                  ...
                  <td className="p-4">
                    {p.variants && p.variants.length > 0 ? (
                      <span title={`${p.variants.length} variantes`}>
                        {p.total_variant_stock ?? 0}
                        <span className="text-ink/40 text-xs ms-1">({p.variants.length})</span>
                      </span>
                    ) : (
                      <span className="text-amber-700 text-xs" title="Aucune variante configurée — le produit ne peut pas être ajouté au panier">
                        ⚠ Aucune variante
                      </span>
                    )}
                  </td>
