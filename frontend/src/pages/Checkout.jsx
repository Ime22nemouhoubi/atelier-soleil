  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await submitOrder({
        ...form,
        items: items.map((i) => ({ product_id: i.product_id, quantity: i.quantity, variant_id: i.variant_id || null })),
      });
      clear();
        ...
        <aside className="bg-sand/40 rounded-2xl p-6 h-fit border border-rose-100">
          ...
          <div className="space-y-3 mb-5 max-h-[300px] overflow-y-auto">
            {items.map((item) => {
              ...
              const variantLabel = [item.size, item.color_name].filter(Boolean).join(' · ');
              return (
                <div key={item.cart_key} className="flex justify-between text-sm">
                  <span className="flex-1 truncate pe-2">
                    {name}
                    {variantLabel && <span className="text-ink/50 ms-1">({variantLabel})</span>}
