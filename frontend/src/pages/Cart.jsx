    ...
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-12 fade-in">
      ...
      <div className="space-y-4">
        {items.map((item) => {
          ...
          const maxQty = item.variant_stock ?? 99;
          return (
            <div
              key={item.cart_key}
            ...
            >
              ...
              <div className="flex-1 text-center sm:text-start">
                ...
                {(item.size || item.color_name) && (
                  <div className="text-xs uppercase tracking-wider text-ink/60 mt-1 flex flex-wrap gap-x-3 gap-y-1 justify-center sm:justify-start">
                    {item.size && <span>{t('product_size')}: <span className="text-ink normal-case tracking-normal">{item.size}</span></span>}
                    {item.color_name && (
                      <span className="inline-flex items-center gap-1.5">
                        {t('product_color')}:
                        {item.color_hex && (
                          <span
                            className="inline-block w-3 h-3 rounded-full border border-rose-200"
                            style={{ backgroundColor: item.color_hex }}
                          />
                        )}
                        <span className="text-ink normal-case tracking-normal">{item.color_name}</span>
                      </span>
                    )}
                  </div>
                )}
              ...
              <div className="flex items-center border border-rose-200 rounded-full overflow-hidden">
                ...
                <button
                  onClick={() => updateQuantity(item.cart_key, Math.min(maxQty, item.quantity + 1))}
                  className="px-3 py-2 hover:bg-rose-50 disabled:opacity-40"
