...
router.post('/', (req, res) => {
  ...
  const variantStmt = db.prepare('SELECT * FROM product_variants WHERE id = ? AND product_id = ?');
  const productHasVariantsStmt = db.prepare('SELECT COUNT(*) AS n FROM product_variants WHERE product_id = ?');
  ...
  for (const it of items) {
    ...
    const line = { product_id: p.id, name: p.name_fr, price: p.price, quantity: qty };

    // Variant enforcement: if the product has any variants, the client MUST pick one
    const variantCount = productHasVariantsStmt.get(p.id).n;
    if (variantCount > 0) {
      const variantId = it.variant_id ? Number(it.variant_id) : null;
      if (!variantId) {
        return res.status(400).json({ error: `Veuillez choisir une taille et une couleur pour "${p.name_fr}"` });
      }
      const variant = variantStmt.get(variantId, p.id);
      if (!variant) {
        return res.status(400).json({ error: `Variante indisponible pour "${p.name_fr}"` });
      }
      if (variant.stock < qty) {
        return res.status(400).json({
          error: `Stock insuffisant pour "${p.name_fr}" (${variant.size} / ${variant.color_name}) — reste ${variant.stock}`,
        });
      }
      line.variant_id = variant.id;
      line.size = variant.size;
      line.color_name = variant.color_name;
      line.color_hex = variant.color_hex;
    }
  ...
  const insertItem = db.prepare(
    `INSERT INTO order_items (order_id, product_id, product_name, unit_price, quantity, variant_id, size, color_name, color_hex)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );
  ...
  const tx = db.transaction(() => {
    ...
    for (const v of validated) {
      insertItem.run(
        orderId, v.product_id, v.name, v.price, v.quantity,
        v.variant_id || null, v.size || null, v.color_name || null, v.color_hex || null
      );
      if (v.variant_id) {
        // Atomic decrement — if two orders race for the last item, the second one fails cleanly.
        const r = decrementVariantStock.run(v.quantity, v.variant_id, v.quantity);
        if (r.changes === 0) {
          throw new Error(`Stock épuisé pendant la commande pour "${v.name}"`);
        }
      }
    }
    return orderId;
